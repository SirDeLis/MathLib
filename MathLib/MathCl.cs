...
using System.Linq;
using System.Text;
using System.Threading.Tasks; //неиспользуемые библиотеки
...
namespace MathCl
{
    public class MathLib
    {
        public class MathCl
        {
            ...
            { //нет проверки на корректность ввода
                double sum = 0;
        ...
        public class Arranges
        {
            ...
            {
                double finded = arr[0]; //кол-во найденных чисел может превышать 1 число
                int findedIndex = 0;//кол-во найденных индексов может превышать 1 число
                for (int i = 0; i < arr.Length; i++)
                {
                    if (dir)
                    {
                        if (finded < arr[i])
                        {
                            finded = arr[i];// повторная проверка на повторение максимального числа и сохранение его в массив
                            findedIndex = i;
                        }
                    }
                    else
            ...
            public static double ArrDet(double[,] arr) // следует добавить проверку на нулевую матрицу
            {
                double det;
                ...
                else if (arr.GetLength(0) == 4 && arr.GetLength(1) == 4) //очень массивный код
                {
                ...
                else //не предусмотрено решение матриц 1х1
                {
                    return Convert.ToDouble(null);
                }
            }
            public static double[,] MatrixSum(double[,] arr1, double[,] arr2) // следует добавить проверку на нулевую матрицу
            {
                if (arr1.GetLength(0) == arr2.GetLength(0) && arr1.GetLength(1) == arr2.GetLength(1))
                {
                    for (int i = 0; i < arr1.GetLength(0); i++)
                    {
                        for (int y = 0; y < arr1.GetLength(1); y++) //отсутсвует сообщение об ошибке, если матрицы нельзя сложить
                        {
                            arr1[i, y] += arr2[i, y];
                        }
                    }
            ...
            public static double[,] MatrixMul(double[,] arr1, double[,] arr2) // следует добавить проверку на нулевую матрицу
            {
                double[,] arr = { { 0 }, { 0 } }; //отсутсвует сообщение об ошибке, если матрицы нельзя перемножить
