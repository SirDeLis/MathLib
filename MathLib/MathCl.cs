...
namespace MathLib
{
    ...
    public class Arranges
    {
        ...
        public static double ArrDet(double[,] arr)  //нахождение определителя
        {
            //нелучший метод решения
            //описаны не все возможные варианты матриц
            double det;
        ...
        {
            double[,] arr = { { 0 }, { 0 } };
            if (arr1.GetLength(0) == arr2.GetLength(1)) // неверный подсчёт матриц а также проверка условия
            {
                arr = new double[arr1.GetLength(0), arr2.GetLength(1)]; //неправильное составление массива
                for (int i = 0; i < arr.GetLength(0); i++)
                {
                    for (int y = 0; y < arr.GetLength(1); y++)
                    {
                        arr[i, y] = 0;
                        for (int k = 0; k < arr1.GetLength(1); k++) //неправильные условия проверки
                        {
